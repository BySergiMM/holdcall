...
nothing here touches the real install.
    ...
    NIM_HOME=<that dir>/nim-home HOME=<that dir>/home engine/bin/nim console

Needs the relay rig's venv (tools/relay-rig/README.md), whose server is the
downstream MCP server the sessions talk to. NIM_BINARY overrides the binary.
...
NIM = os.environ.get("NIM_BINARY") or str(ROOT / "engine/bin/nim")
SERVER = str(ROOT / "tools/relay-rig/server.py")
PY = str(ROOT / "tools/relay-rig/.venv/bin/python")
if len(sys.argv) != 2:
    sys.exit("usage: stage.py <empty directory for NIM_HOME and HOME>")
DEMO = Path(sys.argv[1]).resolve()
HOME = DEMO / "nim-home"; FAKEHOME = DEMO / "home"
...
env = dict(os.environ, NIM_HOME=str(HOME), HOME=str(FAKEHOME))


def nim(*args):
    p = subprocess.run([NIM, *args], env=env, capture_output=True, text=True)
    print("$ nim", " ".join(args), "->", (p.stdout + p.stderr).strip()[:160].replace("\n", " | "))
    return p.stdout
...
class Session:
    ...
    def __init__(self, connector, client):
        self.p = subprocess.Popen(
            [NIM, "serve", "--connector", connector, "--client", client, "--", PY, SERVER],
...
nim("policy", "deny", "explode")
nim("policy", "deny", "delete_repository", "--connector", "github")
nim("policy", "ask", "dangerous_tool")
nim("policy", "ask", "big", "--agent", "claude-code")
nim("policy", "allow", "echo", "--connector", "github")
nim("policy", "budget", "4", "--tool", "add")
nim("policy", "budget", "50", "--all-tools", "--connector", "github")
...
for i in held_ids():
    nim("reject", i, "--reason", "not that branch")
