...
| **Enrolment changes are not journaled** | medium | A rule is scoped to a name; the name's binding to an executable can change with no chain entry (F-018), and now that a rule can grant, not only restrict, that binding is worth more than it was. |
